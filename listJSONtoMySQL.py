...
def getHashtagRelationCount(session):
        ...
        # return hrquery         
        hrquery = session.query(func.max(HashtagRelation.hashtag_relation_id).label('max_hr_id')).\
                                filter(HashtagRelation.dataset_id==twitter_id)
        max_hr_id = hrquery.one().max_hr_id
        print(max_hr_id)
        sys.stdout.flush()
        return max_hr_id
...
def main(user=sys.argv[1],pword=sys.argv[2],db=sys.argv[3],queriesFolder=sys.argv[4]):
        ...
        # now get all query files in the folder loaded into db

...
def generateIncrementers(tweet_dict, author_dict,hashtag_dict,hr_count,um_count):
    ...
    tweet_inc = Incrementer(getMaxFromDict(tweet_dict))
    author_inc = Incrementer(getMaxFromDict(author_dict))
    # text_inc = Incrementer(len(tweet_list))
    hashtag_inc = Incrementer(getMaxFromDict(hashtag_dict))
...
def getHashtagRelationCount(session):
    ...
    # return hrquery         
    query = session.query(func.max(HashtagRelation.hashtag_relation_id).label('max_hr_id')).\
                filter(HashtagRelation.dataset_id==twitter_id)
    max_hr_id = query.one().max_hr_id
    return max_hr_id
...
def getUserMentionCount(session):
    ...
    # return umquery
    query = session.query(func.max(UserMention.user_mention_id).label('max_um_id')).\
                filter(UserMention.dataset_id==twitter_id)
    max_um_id = query.one().max_um_id
    return max_um_id
...
def main(user=sys.argv[1],pword=sys.argv[2],db=sys.argv[3]):
    ...
    # now get all query files
    all_files = getFilesFromFolder(queriesFolder)
    for dataFile in all_files:    
        print('Loading data from',dataFile)
        with open(queriesFolder+'/'+dataFile,'r', encoding='utf-8') as data:
            jObjs = []
            comment_index = 1
            for line in data:
